...
(function (window) {
  ...
  tlm.global = {
    ...
    _handleInteractionRequired: async function (error) {
      ...
      // ตรวจสอบว่าเป็น mobile device และ Safari Mobile หรือไม่
      const isMobile = this._isMobileDevice();
      const isSafariMobile = this._isIOSSafari();

      // ===================================================
      // Safari Mobile: ใช้ POPUP เท่านั้น (ไม่มี redirect)
      // ===================================================
      if (isSafariMobile) {
        console.log('[TLM][Safari Mobile] Detected - using POPUP authentication (NO RELOAD)');

        // 🚫 Check if already showing popup guidance - prevent duplicate attempts
        if (this._showingPopupGuidance) {
          console.log('[TLM][Safari Mobile] ⏸️ Popup guidance is showing - skipping duplicate authentication attempt');
          return null;
        }

        // 🚫 Check if already authenticating - prevent duplicate attempts
        if (this._authenticationInProgress) {
          console.log('[TLM][Safari Mobile] ⏸️ Authentication already in progress - skipping duplicate attempt');
          return null;
        }
      ...
      // ===================================================
      // Desktop & Non-Safari Mobile: ใช้ REDIRECT
      // ===================================================
    ...
    // ===================================================
    // Safari Mobile Popup Authentication Handler (NO RELOAD)
    // ===================================================
    ...
    /**
     * Show platform and browser-specific popup blocker guidance
     */
    _showPopupBlockerGuidance: async function () {
      const platform = this._detectPlatform();
      const browser = this._detectBrowser();

      console.log('[TLM] Showing popup guidance for:', platform, browser);

      // Get guidance content based on platform/browser
      const guidance = this._getPopupGuidanceContent(platform, browser);

      // Show modal dialog
      await this._showGuidanceModal(guidance);
    },

    /**
     * Get popup guidance content for specific platform/browser
     */
    _getPopupGuidanceContent: function (platform, browser) {
      const key = `${platform}_${browser}`;

      const guidanceMap = {
        // iOS Safari
        'ios_safari': {
          title: '🔔 กรุณาปิด Block Pop-ups ใน iOS Safari',
          info: '<strong>ℹ️ ทำไมต้องอนุญาต Pop-up?</strong><br>ระบบต้องการเปิดหน้าต่างเล็กๆ เพื่อให้คุณยืนยันตัวตนอย่างปลอดภัย',
          steps: [
            '1. เปิดแอป <strong>Settings</strong> (⚙️) บน iPhone/iPad',
            '2. เลื่อนลงหา <strong>Safari</strong>',
            '3. ในส่วน "General" หา <strong>"Block Pop-ups"</strong>',
            '4. <strong>ปิด</strong> (toggle เป็นสีเทา)',
            '5. กลับมาที่หน้าเว็บนี้',
            '6. กด <strong>"ลองอีกครั้ง"</strong> ด้านล่างเพื่อดำเนินการต่อ'
          ],
          note: '⚠️ การตั้งค่านี้จะมีผลกับการเข้าใช้งานระบบนี้เท่านั้น ไม่กระทบการใช้งาน Safari ทั่วไป'
        },

        // Generic fallback
        'generic': {
          title: '🔔 กรุณาอนุญาต Pop-up Window',
          info: '<strong>ℹ️ ทำไมต้องอนุญาต Pop-up?</strong><br>ระบบต้องการเปิดหน้าต่างเล็กๆ เพื่อให้คุณยืนยันตัวตนอย่างปลอดภัย',
          steps: [
            '1. เปิด <strong>Settings</strong> ของ browser',
            '2. ค้นหา <strong>"Pop-up"</strong> หรือ <strong>"Site Settings"</strong>',
            '3. อนุญาต pop-ups สำหรับ <strong>thaioilgroup0.sharepoint.com</strong>',
            '4. Refresh หน้าเว็บ',
            '5. กด <strong>"ลองอีกครั้ง"</strong> ด้านล่างเพื่อดำเนินการต่อ'
          ],
          note: '💡 ขั้นตอนอาจแตกต่างกันไปตาม browser ที่ใช้งาน'
        }
      };

      return guidanceMap[key] || guidanceMap['generic'];
    },
    ...
    _detectPlatform: function () {
      ...
      if (this._isAndroidDevice()) return 'android';
    ...
    _detectBrowser: function () {
      ...
      // Check for Safari (iOS and Desktop)
      if (/Safari/i.test(ua) && !/Chrome/i.test(ua) && !/CriOS/i.test(ua) && !/EdgiOS/i.test(ua)) {
        return 'safari';
      }
    ...
    /**
     * Show guidance modal dialog and wait for user action
     * Returns a Promise that resolves when user clicks retry button
     */
    _showGuidanceModal: function (guidance) {
      return new Promise((resolve) => {
        // Create modal overlay
        const overlay = document.createElement('div');
        overlay.id = 'tlm-popup-guidance-overlay';
        ...
        // Build content
        let stepsHTML = guidance.steps.map(step => `<li style="margin-bottom: 12px; line-height: 1.6;">${step}</li>`).join('');

        modal.innerHTML = `
          <h2 style="margin: 0 0 24px 0; color: #d83b01; font-size: 24px; font-weight: 600;">
            ${guidance.title}
          </h2>
          <div style="background: #fff4e5; border-left: 4px solid #ffb900; padding: 16px; margin-bottom: 24px; border-radius: 4px;">
            <p style="margin: 0; color: #3b3a39; line-height: 1.6;">
              ${guidance.info || '<strong>ℹ️ ทำไมต้องอนุญาต Pop-up?</strong><br>ระบบต้องการเปิดหน้าต่างเล็กๆ เพื่อให้คุณยืนยันตัวตนอย่างปลอดภัย'}
            </p>
          </div>
          <div style="background: #f3f2f1; padding: 20px; border-radius: 8px; margin-bottom: 24px;">
            <h3 style="margin: 0 0 16px 0; color: #323130; font-size: 18px;">📝 วิธีการตั้งค่า:</h3>
            <ol style="margin: 0; padding-left: 24px; color: #605e5c;">
              ${stepsHTML}
            </ol>
          </div>
          ${guidance.alternative ? `
            <div style="background: #e1f5fe; border-left: 4px solid #0078d4; padding: 16px; margin-bottom: 24px; border-radius: 4px;">
              <p style="margin: 0; color: #3b3a39; line-height: 1.6;">
          ...
          ` : ''}
          ${guidance.note ? `
            <div style="background: #fff4ce; border-left: 4px solid #ff8c00; padding: 16px; margin-bottom: 24px; border-radius: 4px;">
              <p style="margin: 0; color: #3b3a39; line-height: 1.6;">
                ${guidance.note}
              </p>
            </div>
          ` : ''}
          <div style="background: #ffebee; border: 2px solid #d32f2f; padding: 16px; margin-bottom: 24px; border-radius: 8px;">
            <label style="display: flex; align-items: center; cursor: pointer; user-select: none;">
              <input type="checkbox" id="tlm-confirm-checkbox" style="
                width: 20px;
                height: 20px;
                margin-right: 12px;
                cursor: pointer;
              ...
              <span style="color: #d32f2f; font-weight: 600; font-size: 15px;">
                ✓ ฉันได้ปิด Block Pop-ups ใน Settings แล้ว
              </span>
            </label>
          </div>
          <div style="display: flex; gap: 12px; justify-content: flex-end;">
            <button id="tlm-popup-retry-btn" disabled style="
              background: #cccccc;
              color: white;
              border: none;
              padding: 12px 32px;
        ...
        // Get elements
        const checkbox = document.getElementById('tlm-confirm-checkbox');
        const retryBtn = document.getElementById('tlm-popup-retry-btn');
        ...
        checkbox.addEventListener('change', () => {
          if (checkbox.checked) {
            ...
            retryBtn.onmouseover = () => retryBtn.style.background = '#106ebe';
            retryBtn.onmouseout = () => retryBtn.style.background = '#0078d4';
          } else {
            retryBtn.disabled = true;
            retryBtn.style.background = '#cccccc';
            retryBtn.style.cursor = 'not-allowed';
            retryBtn.onmouseover = null;
            retryBtn.onmouseout = null;
          }
        });

        // Handle retry button - only works when enabled
        retryBtn.addEventListener('click', () => {
          if (!retryBtn.disabled) {
            document.body.removeChild(overlay);
            resolve(); // Only resolves when user clicks retry!
          }
        });

        console.log('[TLM] Popup guidance modal displayed');
      });
    },
    ...
    _performSafariPopupLogin: async function () {
      console.log('[TLM][Safari Mobile] 🚀 Performing popup authentication (NO RELOAD)...');

      try {
        // 🔥 CRITICAL: Clear ALL MSAL interaction locks before popup
        console.log('[TLM][Safari Mobile] 🗑️ Clearing all MSAL interaction locks...');
        ...
        if (lockKeys.length > 0) {
          console.log(`[TLM][Safari Mobile] 🗑️ Found ${lockKeys.length} MSAL lock keys:`, lockKeys);
          lockKeys.forEach(key => {
            localStorage.removeItem(key);
            console.log(`[TLM][Safari Mobile] 🗑️ Removed: ${key}`);
          });
        }

        // Also clear sessionStorage locks
        ...
        if (sessionLockKeys.length > 0) {
          console.log(`[TLM][Safari Mobile] 🗑️ Found ${sessionLockKeys.length} sessionStorage lock keys`);
          sessionLockKeys.forEach(key => {
            sessionStorage.removeItem(key);
            console.log(`[TLM][Safari Mobile] 🗑️ Removed from session: ${key}`);
          });
        }

        // Small delay to ensure locks are fully cleared
        await new Promise(resolve => setTimeout(resolve, 500));
        ...
        if (popupResult && popupResult.accessToken) {
          ...
          if (this.checkCurrentTokenStatus) {
            ...
            console.log('[TLM][Safari Mobile] ✅ Token status:', status);
          }

          console.log('[TLM][Safari Mobile] ✅ Authentication complete - NO RELOAD, ready to use');
          return true;

        } else {
          console.error('[TLM][Safari Mobile] ❌ No access token in result');
          return false;
        }

      } catch (error) {
        console.error('[TLM][Safari Mobile] ❌ Popup authentication error:', error);

        // Check error type
        if (error.errorCode === 'user_cancelled') {
          console.log('[TLM][Safari Mobile] User cancelled popup');
          return false;
        }

        if (error.errorCode === 'popup_window_error' ||
          (error.message && error.message.includes('popup'))) {
          console.warn('[TLM][Safari Mobile] Popup blocked error');
          // Will retry from _safariMobilePopupAuth
          return false;
        }

        console.error('[TLM][Safari Mobile] Unexpected error:', error);
        return false;
      }
    },
        ...
        system: {
          loggerOptions: {
            loggerCallback: (level, message, containsPii) => {
              ...
              if (shouldSuppress && (this._isIOSSafari() || this._isMobileDevice())) {
          ...
          windowHashTimeout: this._isIOSSafari() ? 180000 : (this._isMobileDevice() ? 120000 : 60000),
          iframeHashTimeout: this._isIOSSafari() ? 180000 : (this._isMobileDevice() ? 120000 : 60000),
          loadFrameTimeout: this._isIOSSafari() ? 180000 : (this._isMobileDevice() ? 120000 : 60000),
          allowNativeBroker: false,
          allowRedirectInIframe: !this._isMobileDevice(), // ปิดการใช้ iframe บน mobile
          networkRequestTimeout: this._isIOSSafari() ? 90000 : (this._isMobileDevice() ? 60000 : 30000),
          telemetry: {
        ...
        try {
          ...
          if (response) {
            ...
            if (response.accessToken) {
              ...
              // สำหรับ Safari Mobile: รอให้ token พร้อมก่อน app เรียก ajax
              if (this._isIOSSafari()) {
                await new Promise(resolve => setTimeout(resolve, 500));
